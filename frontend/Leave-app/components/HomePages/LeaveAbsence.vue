<template>
    <div class="leave-form">
      <div class="leave-credits">
        <div v-for="(leave, index) in leaveCredits" :key="index" :class="['leave-credit', leave.class]">
          <div>{{ leave.remaining }}</div>
          <div>วัน/{{ leave.total }}</div>
          <div>{{ leave.label }}</div>
        </div>
      </div>
  ...
  export default {
    data() {
      return {
        leaveCredits: [
          { label: 'สิทธิ์ลาป่วย', remaining: 19, total: 30, class: 'sick-leave' },
          { label: 'สิทธิ์ลากิจ', remaining: 19, total: 30, class: 'business-leave' },
          { label: 'สิทธิ์ลาพักร้อน', remaining: 19, total: 30, class: 'vacation-leave' },
        ],
        leaveTypes: [
